...
class _CreateRoleModalState extends ConsumerState<CreateRoleModal> 
  ...
  Future<void> _createRole() async {
    ...
    try {
      ...
      final roleId = await ref.read(roleServiceProvider).createRole(
      ...
      if (roleId != null && _selectedTags.isNotEmpty) {
        await ref.read(roleServiceProvider).updateRole(
          roleId: roleId,
          roleName: _roleNameController.text.trim(),
          featureIds: _selectedFeatureIds.keys.where((id) => _selectedFeatureIds[id]!).toList(),
          description: _descriptionController.text.trim().isEmpty 
      ...
      if (mounted) {
        ref.invalidate(companyRolesProvider(widget.companyId));
                ...
                data: (features) => Column(
                  children: [
                    ...categories.map((category) {
                      ...
                      if (!_categoryKeys.containsKey(category.categoryId)) {
  ...
  Widget _buildPermissionCategory(Category category, List<Feature> features) {
    final isExpanded = _expandedCategories[category.categoryId] ?? false;
    final selectedCount = features.where((f) => 
      _selectedFeatureIds[f.featureId] ?? false
    ).length;
              ...
              onTap: () {
                ...
                if (!isExpanded) {
                  ...
                  WidgetsBinding.instance.addPostFrameCallback((_) {
                    final context = _categoryKeys[category.categoryId]?.currentContext;
              ...
              child: AnimatedContainer(
                ...
                children: [
                  ...
                  Material(
                    ...
                    child: InkWell(
                      onTap: () => _toggleCategory(category.categoryId, features),
                ...
                ? Column(
                    children: [
                      ...
                      ...features.map((feature) {
                        final isSelected = _selectedFeatureIds[feature.featureId] ?? false;
                        return Material(
                          color: Colors.transparent,
                          child: InkWell(
                            onTap: () {
                              setState(() {
                                _selectedFeatureIds[feature.featureId] = !isSelected;
                              });
                            },
                            child: AnimatedContainer(
  ...
  void _toggleAllPermissions(List<Feature> features) {
    setState(() {
      ...
      if (allSelected) {
        ...
        for (final feature in features) {
          _selectedFeatureIds[feature.featureId] = true;
        }
      }
    });
  }

  void _toggleCategory(String categoryId, List<Feature> categoryFeatures) {
    setState(() {
      ...
      for (final feature in categoryFeatures) {
        _selectedFeatureIds[feature.featureId] = !allSelected;
      }
    });
  }
            ...
            child: SafeArea(
              child: TossButton(
                ...
                onPressed: _canProceed 
                    ...
                    : null,
                type: TossButtonType.filled,
                size: TossButtonSize.large,
                isLoading: _isCreating,
                leadingIcon: _currentStep < 2 
